...
class Text(Shape):
    def __init__(self, x: Union[int, str], y: Union[int, str], text: str, 
                parent: Union[Canvas, Link], fill: str = "black", 
                font_size: str = "16px", transform: Optional[str] = ""):
    ...
    def svg_content(self) -> str:
        return f'<text x="{self.x}" y="{self.y}" fill="{self.fill}" transform="{self.transform}" dominant-baseline="middle" text-anchor="middle" font-size="{self.font_size}">{self.text}</text>'
